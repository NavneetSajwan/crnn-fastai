...
class strLabelConverter(object):
    ...
    def encode(self, text):
        ...
        result = []
        for item in text:  
            item = item.decode('utf-8','strict') # item = पार्टी
            # print(item)          
            length.append(len(item)) # length = [..,6, ]
            r = []
            for char in item:
    ...
    def encoder(self, label):
        ...
        # print('label in hindi', label)
        length = len(label)
        result = []
        r=[]
        # print('hindi label', label)
        ...
        # result_temp.append(r)
        # text = result_temp
        return torch.tensor(result), torch.tensor(length)
